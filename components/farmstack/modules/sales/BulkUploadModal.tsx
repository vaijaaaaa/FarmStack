...
interface BulkUploadModalProps {
  ...
  // maps them onto the new-sale form. Unit / GST / Tally price come from the
  // product master + latest purchase there.
  onApply: (rows: ParsedSalesRow[]) => void
  products: { id: string; name: string }[]
}

const EXPECTED_HEADERS = ['product_name', 'quantity', 'selling_price', 'expiry_date']

const TEMPLATE_CSV =
  'product_name,quantity,selling_price,expiry_date\n' +
  'Urea,10,300,2027-12-31\n' +
  'DAP,5,1400,2027-06-30'

export default function BulkUploadModal({ isOpen, onClose, onApply, products }: BulkUploadModalProps) {
  const [file, setFile] = useState<File | null>(null)
  const [preview, setPreview] = useState<ParsedSalesRow[]>([])
  ...
  const rowWarning = (row: ParsedSalesRow): string => {
  ...
  const parseCSV = (file: File) => {
    ...
    reader.onload = (event) => {
      ...
      if (!headersMatch) {
        toast.error(
          `Invalid columns. This must be a sales CSV with columns: ${EXPECTED_HEADERS.join(', ')}`,
        )
        setFile(null)
        setPreview([])
        return
      }
      const rows: ParsedSalesRow[] = lines.slice(1).map((line) => {
        const [productName, quantity, sellingPrice, expiryDate] = line
          .split(',')
          .map((s) => (s || '').trim())
        return {
  ...
  const updateRow = (idx: number, field: keyof ParsedSalesRow, value: string) => {
  ...
  const handleApply = () => {
    ...
    toast.success(
      `${valid.length} item(s) added to sale` + (skipped > 0 ? `, ${skipped} skipped` : ''),
    )
    onClose()
  }
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-lg bg-white p-6">
        <h2 className="mb-4 text-2xl font-bold text-black">Bulk Upload Sale Items</h2>

        <a
          href={`data:text/csv;charset=utf-8,${encodeURIComponent(TEMPLATE_CSV)}`}
          download="sale_items_template.csv"
        ...
        <div className="mb-4 rounded-lg border-2 border-dashed border-gray-300 p-8 text-center">
          <input
            ...
            id="sale-csv-upload"
          />
          <label htmlFor="sale-csv-upload" className="cursor-pointer">
